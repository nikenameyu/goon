...
func CheckPar(){
	...
	*/
	if public.InputValue.IpsPtr !=""{
		...
		if len(lines) == 0{
			...
			*/
			if  public.InputValue.ModePtr == "title" || public.InputValue.ModePtr == "dir" || public.InputValue.ModePtr == "finger" || public.InputValue.ModePtr == "tomcat"{
		...
		if public.InputValue.ModePtr != "fofa" && public.InputValue.ModePtr != "hunter"{
	...
	*/
	...
	if public.InputValue.HeaderPtr != ""{
		public.ConfValue.DirInfo.Header = public.InputValue.HeaderPtr
	}
	if public.InputValue.CodePtr != 200 {
		public.ConfValue.DirInfo.Code = public.InputValue.CodePtr
	}
	/*
	fofa的一些参数
	- num: api返回数据量
...
/*
对某个段进行探活的时候 先探测该段的第一个和最后一个地址
如：192.168.1.0/24 --> 优先探活192.168.0.1 和 192.168.0.255
*/
func CheckSub(ips string) bool{
	ipsub := getSubNet(ips)
	if ipsub != "err"{

		ips_Arr := strings.SplitN(ipsub,"-",-1)
		if scan.IcmpScan(ips_Arr[0],3) || scan.IcmpScan(ips_Arr[1],3){
			return true
		}
	}
	return false
